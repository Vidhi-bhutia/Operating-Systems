...
int main() {
    ...
    if (direction == 0) {
        ...
        // Add seek time to move to the last request when moving left
        ...
        seek_time += abs(head - (disk_size - 1)); // Move to the rightmost track
        printf("Moved from %d to %d\n", head, disk_size - 1);
        head = disk_size - 1;
        for (i = index - 1; i >= 0; i--) {
            seek_time += abs(requests[i] - head);
        ...
        // Add seek time to move to the first request when moving right
